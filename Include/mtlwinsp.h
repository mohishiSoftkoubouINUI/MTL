...
class CSplitterWnd : public CWnd
{
	...
	public:
		CSplitterWnd() : afxData(CAfx::GetAfxData())
		{
	...
	public:
		...
		void OnCancelMode() { StopTracking(FALSE); }
		...
		virtual void OnDraw(CDC* pDC)
		{
			...
			if (m_bHasVScroll && m_nRows < m_nMaxRows)
			{
				OnDrawSplitter(pDC, splitBox,
					CRect(rectInside.right, rectClient.top, rectClient.right, rectClient.top + m_cySplitter));
			}

			if (m_bHasHScroll && m_nCols < m_nMaxCols)
			{
				OnDrawSplitter(pDC, splitBox,
					CRect(rectClient.left, rectInside.bottom, rectClient.left + m_cxSplitter, rectClient.bottom));
			}
		...
		virtual void OnInvertTracker(const CRect& rect)
		{
			...
			if (pBrush != NULL)
				hOldBrush = (HBRUSH)SelectObject(pDC->m_hDC, pBrush->m_hBrush);
		...
		void OnLButtonUp(UINT nFlags, CPoint pt) { StopTracking(TRUE); }
		...
		virtual CWnd* GetActivePane(int& /*row*/, int& /*col*/)
		{
		...
		virtual void GetHitRect(int ht, CRect& rectHit)
		{
			ASSERT_VALID(this);
		...
		BOOL IsTracking() { return m_bTracking; }// TRUE during split operation

		// Generic routine:
		//  for X direction: pInfo = m_pColInfo, nMax = m_nMaxCols, nSize = cx
		//  for Y direction: pInfo = m_pRowInfo, nMax = m_nMaxRows, nSize = cy
		void _AfxLayoutRowCol(CSplitterWnd::CRowColInfo* pInfoArray, int nMax, int nSize, int nSizeSplitter)
		{
		...
		void _AfxDeferClientPos(AFX_SIZEPARENTPARAMS* lpLayout, CWnd* pWnd, int x, int y, int cx, int cy, BOOL bScrollBar)
		{
			ASSERT(pWnd != NULL);
		...
		virtual void RecalcLayout()    // call after changing sizesvoid CSplitterWnd::RecalcLayout()
		{
			ASSERT_VALID(this);
		...
		virtual void SetSplitCursor(int ht)
		{
			...
			static HCURSOR _afx_hcurDestroy = NULL;
			static UINT _afx_idcPrimaryLast = 0; // store the primary IDC
			...
			if (idcPrimary != 0)
			{
				...
				{
					HINSTANCE hInst = AfxFindResourceHandle(ATL_MAKEINTRESOURCE(idcPrimary), ATL_RT_GROUP_CURSOR);
					...
					if ((_afx_hcurDestroy = _afx_hcurLast = ::LoadCursorW(hInst, ATL_MAKEINTRESOURCEW(idcPrimary))) == NULL)
					{
		...
		static int _AfxCanSplitRowCol(CSplitterWnd::CRowColInfo* pInfoBefore, int nBeforeSize, int nSizeSplitter)
...
_INLINE CSplitterWnd* CView::GetParentSplitter(CWnd* pWnd, BOOL bAnyState)
{
	ENSURE_VALID(pWnd);
...
_INLINE void _AFX_MOUSEANCHORWND::OnTimer(UINT_PTR nIDEvent)
{
	...
	else
	{
		...
		HINSTANCE hInst = AfxFindResourceHandle(ATL_MAKEINTRESOURCE(nCursor), ATL_RT_GROUP_CURSOR);
		...
		SetWindowPos(&CWnd::wndTop,
					m_ptAnchor.x - AFX_CX_ANCHOR_BITMAP / 2, m_ptAnchor.y - AFX_CY_ANCHOR_BITMAP / 2, 0, 0,
